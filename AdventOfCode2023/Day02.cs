...
public class Day02
{
    ...
    public void Part_1_Example()
        => SumOfPossible(exampleInput).Should().Be(8);

    [Fact]
    public void Part_1_Puzzle()
        => SumOfPossible(myInput).Should().Be(1734);

    [Fact]
    public void Part_2_Example()
        => PowerSum(exampleInput).Should().Be(2286);

    [Fact]
    public void Part_2_Puzzle()
        => PowerSum(myInput).Should().Be(70387);

    static Option<int> SumOfPossible(string games)
        => GamesParser.Parse(games)
            .ToOption()
            .FilterT(IsPossible)
            .MapT(g => g.Id)
            .Map(ids => ids.Sum());

    static Option<int> PowerSum(string games)
        => GamesParser.Parse(games)
            .ToOption()
            .MapT(MinimumCubesPossible)
            .MapT(Power)
            .Map(powers => powers.Sum());

    static Cubes MinimumCubesPossible(Game game)
        => game.Rounds.Fold(
            new Cubes(0, 0, 0),
            (s, e) => new Cubes(
                Math.Max(s.Red, e.Red),
                Math.Max(s.Green, e.Green),
                Math.Max(s.Blue, e.Blue)));

    static int Power(Cubes cubes)
        => cubes.Red * cubes.Green * cubes.Blue;

    static bool IsPossible(Game game)
        => game.Rounds.ForAll(r =>
        r.Red <= 12
        && r.Green <= 13
        && r.Blue <= 14);

    static Parser<Seq<Game>> GamesParser
        => sepEndBy(GameParser, endOfLine);

    static Parser<Game> GameParser
        => from _1 in str("Game")
           from _2 in space
           from id in asInteger(many(digit)).Flatten(() => "not a number")
           from _3 in ch(':')
           from rounds in sepBy(RoundParser, ch(';'))
           select new Game(id, rounds);

    static Parser<Cubes> RoundParser
        => sepBy(CubeParser, ch(','))
        .Map(r => r.Fold(new Cubes(0, 0, 0), (s, e) => s + e));

    static Parser<Cubes> CubeParser
        => from _1 in space
           from count in asInteger(many(digit)).Flatten(() => "not a number")
           from _2 in space
           from color in asString(many(letter))
           from cubes in color switch
           {
               "red" => result(new Cubes(count, 0, 0)),
               "green" => result(new Cubes(0, count, 0)),
               "blue" => result(new Cubes(0, 0, count)),
               _ => failure<Cubes>($"unknown color {color}")
           }
           select cubes;

    record Game(int Id, Seq<Cubes> Rounds);

    record Cubes(int Red, int Green, int Blue)
    {
        public static Cubes operator +(Cubes a, Cubes b)
            => new Cubes(
                a.Red + b.Red,
                a.Green + b.Green,
                a.Blue + b.Blue);
