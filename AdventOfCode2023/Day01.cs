...
public class Day01
{
    ...
    public void Part_1_Example()
        => SumOfCalibrationValues(exampleInputPart1, CharParserPart1())
        .Should().Be(142);

    [Fact]
    public void Part_1_Puzzle()
        => SumOfCalibrationValues(myInput, CharParserPart1())
        .Should().Be(54644);

    [Fact]
    public void Part_2_Example()
        => SumOfCalibrationValues(exampleInputPart2, CharParserPart2())
        .Should().Be(281);

    [Fact]
    public void Part_2_Puzzle()
        => SumOfCalibrationValues(myInput, CharParserPart2())
        .Should().Be(53348);

    static Option<int> SumOfCalibrationValues(
        string calibrationDocument,
        Parser<char> charParser)
    {
        var digits = charParser
            .Map(Convert.ToString)
            .Map(parseInt)
            .Apply(many)
            .Map(maybeDigits => maybeDigits.Somes());

        var calibrationValue = digits
            .Map(digits => from first in digits.HeadOrNone()
                           from last in digits.LastOrNone()
                           select first * 10 + last)
            .Flatten(() => "one digit is required");

        return sepEndBy(calibrationValue, endOfLine)
            .Parse(calibrationDocument)
            .ToOption()
            .Map(calibrationValues => calibrationValues.Sum());
    }

    static Parser<char> CharParserPart1()
        => alphaNum;

    static Parser<char> CharParserPart2()
    {
        var digitKeyword = choice(
            Digit('1', "one"),
            Digit('2', "two"),
            Digit('3', "three"),
            Digit('4', "four"),
        ...
        return from a in digitKeyword.Apply(lookAhead).Apply(optional)
               from b in alphaNum
               select a.IfNone(b);

        static Parser<char> Digit(char ch, string letters)
            => str(letters).Map(_ => ch).Apply(attempt);
    }

    const string exampleInputPart1 = """
        1abc2
        pqr3stu8vwx
        a1b2c3d4e5f
        treb7uchet
    ...
    const string exampleInputPart2 = """
        two1nine
        eightwothree
        abcone2threexyz
        xtwone3four
