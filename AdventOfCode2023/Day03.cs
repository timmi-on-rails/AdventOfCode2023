...
public class Day03
{
    ...
    public void Part_1_Example()
        => SumOfPartNumbers(exampleInput).Should().Be(4361);

    [Fact]
    public void Part_1_Puzzle()
        => SumOfPartNumbers(myInput).Should().Be(532445);

    [Fact]
    public void Part_2_Example()
        => SumOfGearRatios(exampleInput).Should().Be(467835);

    [Fact]
    public void Part_2_Puzzle()
        => SumOfGearRatios(myInput).Should().Be(79842967);

    static Option<int> SumOfPartNumbers(string input)
        => EngineSchematicParser.Parse(input)
        .ToOption()
        .Map(SumOfPartNumbers);

    static int SumOfPartNumbers(EngineSchematic schema)
    {
        var positions = new HashSet<(int, int)>(schema.Symbols.Collect(s => Seq(
            (s.Line, s.Column),
            (s.Line + 1, s.Column),
            (s.Line - 1, s.Column),
            (s.Line, s.Column - 1),
        ...
        return schema.PartNumbers.Filter(num => num.Digits.Any(d => positions.Contains((d.Line, d.Column))))
            .Sum(num => num.Number);
    }

    static Option<int> SumOfGearRatios(string input)
        => EngineSchematicParser.Parse(input)
        .ToOption()
        .Map(SumOfGearRatios);

    static int SumOfGearRatios(EngineSchematic schema)
    {
        return schema.Symbols.Filter(s => s.Sym == '*')
            .Map(s => schema.PartNumbers.Filter(p
                => p.Digits.Any(d => Math.Abs(d.Line - s.Line) <= 1 && Math.Abs(d.Column - s.Column) <= 1)))
            .Filter(ps => ps.Count == 2)
            .Map(ps => ps.Fold(1, (s, p) => s * p.Number))
            .Sum();
    }

    static Parser<EngineSchematic> EngineSchematicParser
        => many(choice(
            PartNumberParser.Map(Either<Symbol, PartNumber>.Right).Map(Some).Apply(attempt),
            SymbolParser.Map(x => x).Map(Either<Symbol, PartNumber>.Left).Map(Some).Apply(attempt),
            anyChar.Map(_ => Option<Either<Symbol, PartNumber>>.None)))
        .Map(list => list.Fold(
            new EngineSchematic(Empty, Empty),
            (s, e) => e.Match(
                symbolOrPartNumber => symbolOrPartNumber.Match(
                    partNumber => s with { PartNumbers = s.PartNumbers.Add(partNumber) },
                    symbol => s with { Symbols = s.Symbols.Add(symbol) }),
                () => s)));

    static Parser<PartNumber> PartNumberParser
        => many1(attempt(from pos in getPos
                 from digit in digit
                 select new Digit(pos.Line, pos.Column, digit)))
                 .Map(digits => new PartNumber(
                     digits,
                     Convert.ToInt32(digits.Fold(string.Empty, (s, v) => s + v.Char))));
    
    static Parser<Symbol> SymbolParser
        => from pos in getPos
           from symbol in oneOf("#+-/*$@%&=")
           select new Symbol(pos.Line, pos.Column, symbol);

    record EngineSchematic(Seq<Symbol> Symbols, Seq<PartNumber> PartNumbers);

    record Symbol(int Line, int Column, char Sym);

    record PartNumber(Seq<Digit> Digits, int Number);

    record Digit(int Line, int Column, char Char);
